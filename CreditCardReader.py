...
def template_match(image,req_str):
    try:
        # load the reference OCR-A image from disk, convert it to grayscale, and threshold it, such that the digits appear as *white* on a
        ...
        # apply a closing operation using the rectangular kernel to help close gaps in between credit card number digits, then apply
        ...
        for (i, (gX, gY, gW, gH)) in enumerate(locs):
            ...
            for c in digitCnts:
                ...
                # and resize it to have the same fixed size as the reference OCR-A images
                (x, y, w, h) = cv2.boundingRect(c)
